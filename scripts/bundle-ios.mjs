...
for (const entry of await readdir(root, { withFileTypes: true })) {
  const runtimeFile =
    entry.isFile() && /\.(html|css|js|webmanifest|ico)$/.test(entry.name);
  const runtimeFolder =
    entry.isDirectory() &&
    ["assets", "data", "icons", "branding"].includes(entry.name);
  if (runtimeFile || runtimeFolder) {
    await cp(new URL(entry.name, root), new URL(entry.name, target), {
      recursive: true,
    });
  }
}
